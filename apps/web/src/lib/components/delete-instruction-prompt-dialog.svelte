<script>
    ...
    import {
        createDeleteInstructionPromptMutation,
     ...
     * @property {import('$lib/api/prompts').InstructionPromptId} instructionPromptId
    ...
    const { instructionPromptId, onSuccess, ...props } = $props();
    ...
    const deleteMutation = createDeleteInstructionPromptMutation();

    function deleteInstructionPrompt() {
        deleteMutation.mutate([ky, { params: { id: instructionPromptId } }], {
            onError: (err) =>
                toaster.push("Failed to delete instruction prompt", {
...
<AlertDialog bind:ref={dialog} {...props}>
    <AlertDialogContent size="sm">
        <AlertDialogHeader>
            <AlertDialogTitle>Delete instruction prompt?</AlertDialogTitle>
            <AlertDialogDescription>
                This action will delete the instruction prompt. It cannot be undone later.
            </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
            ...
            <AlertDialogContinueAction
                ...
                onclick={deleteInstructionPrompt}
                variant="destructive"
            >
