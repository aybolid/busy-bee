...
use crate::repos::articles::{
    Article, ArticleByLine, ArticleContent, ArticleExcerpt, ArticleId, ArticleLang,
    ArticleSiteName, ArticleStatus, ArticleTextContent, ArticleTitle,
};

/// Errors that can occur when converting a [`dom_smoothie::Article`] into an [`Article`].
#[derive(Debug, thiserror::Error)]
pub enum FromReadabilityArticleError {
...
/// Attempts to map a raw scraped [`dom_smoothie::Article`] into our domain-level [`Article`].
///
/// # Errors
///
/// Returns a [`FromReadabilityArticleError`] if strictly required fields
/// (like `title`, `content`, `text_content`, or `url`) are missing or empty.
impl TryFrom<dom_smoothie::Article> for Article {
    type Error = FromReadabilityArticleError;

    fn try_from(value: dom_smoothie::Article) -> Result<Self, Self::Error> {
        let title = ArticleTitle::new(value.title)
            .ok_or(FromReadabilityArticleError::EmptyString("title"))?;
        let content = ArticleContent::new(value.content.to_string())
            .ok_or(FromReadabilityArticleError::EmptyString("content"))?;
        let text_content = ArticleTextContent::new(value.text_content.to_string())
            .ok_or(FromReadabilityArticleError::EmptyString("text_content"))?;
        ...
        let url = value
            ...
            .ok_or(FromReadabilityArticleError::MissingOrInvalidArticleUrl)?;
